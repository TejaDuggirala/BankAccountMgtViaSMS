...
public class BankAccountMgtApiWebContextTest {
	...
	public void setUp() {
		...
		userDbEntity.setDeviceId("device-TEJA");

		UserDbEntity userDbEntity_recipient = new UserDbEntity();
		userDbEntity_recipient.setUserName("SAI");
		...
		userTranDbEntity.setReceiver("SAI");
		userTranDbEntity.setUserTransc(userDbEntity);

		userTranDbEntity.setTransferredAmount(new BigDecimal(100.00));

		userDao.save(userDbEntity);
		userDao.save(userDbEntity_recipient);
		userTranscationDao.save(userTranDbEntity);
...
/*	@Test
	public void processSMSCommand_SHOW_BALANCE() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=BALANCE&deviceId=device-TEJA")).andExpect(status().isOk())
	...
	public void processSMSCommand_SEND() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=SEND-100-SAI&deviceId=device-TEJA"))
	...
	public void processSMSCommand_TOTAL_SENT() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=TOTAL-SENT-SAI&deviceId=device-TEJA"))
				.andExpect(status().isOk()).andExpect(content().string("100"));
	}

	/*@Test
	public void processSMSCommand_ERR_INVALID_DEVICE() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=BALANCE&deviceId=device-TEJA8"))
	...
	public void processSMSCommand_ERR_NO_USER() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=TOTAL-SENT-SAIA&deviceId=device-TEJA"))
				.andExpect(status().isExpectationFailed())
	...
	public void processSMSCommand_ERR_INSUFFICIENT_FUNDS() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=SEND-10000-SAI&deviceId=device-TEJA"))
				.andExpect(status().isExpectationFailed())
	...
	public void processSMSCommand_INTERNAL_SERVER_ERROR() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=XYZSEND&deviceId=device-TEJA"))
	...
	public void processSMSCommand_ERR_INVALID_COMMAND() throws Exception {
		mockMvc.perform(get("/smsCommand/command?sms=XYZ&deviceId=device-TEJA"))
				.andExpect(status().isExpectationFailed())
